...
You can configure Solr to download exchange rates from a URL endpoint that returns JSON data in the openexchangerates.org format.
You can use https://www.OpenExchangeRates.Org[OpenExchangeRates.Org] or any other compatible service.
These rates are typically symmetrical only.

In this case, you need to specify the `providerClass` in the definitions for the field type.
Here is an example:
...
<fieldType name="currency" class="solr.CurrencyFieldType"
           ...
           refreshInterval="60"
           ratesFileLocation="https://openexchangerates.org/api/latest.json?app_id=yourPersonalAppIdKey"/>
----
...
The `ratesFileLocation` must be an HTTPS URL with a path ending in `/latest.json`, or alternatively you can provide the path to a local JSON file.
The provider accepts URLs from any host, as long as they use HTTPS and the endpoint path ends with `/latest.json`.
